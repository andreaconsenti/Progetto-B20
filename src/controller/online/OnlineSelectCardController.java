...
public class OnlineSelectCardController {
    ...
    public void initialize() {
    	...
    	if(OnlineGameSceneController.game.getGamePhase()==GAME_PHASE.REINFORCEMENT) {
    		paneScambioCarte.setOpacity(1);
    	}
    	
    	cards = new HashMap<Card, CardGui>();
    	
    	for(Card ca : OnlineGameSceneController.game.getCurrentTurn().getCards()) {
    		addCard(ca);
    	}
	...
	public void onScambiaPressed(ActionEvent event) throws IOException {
		OnlineGameSceneController.game.playCardTris(card1, card2, card3);
	...
	private void guiHandler() {
		if((card1 != null) && (card2 != null) && (card3 != null)) {
			...
			if(OnlineGameSceneController.game.checkTris(card1, card2, card3) != 0) {
				scambiaButton.setDisable(false);
				exchangeLabel.setText("Bonus scambio: " + OnlineGameSceneController.game.checkTris(card1, card2, card3));
			} else {
